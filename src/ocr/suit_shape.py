...
def _largest_contour(th):
    ...
    if cv2.contourArea(cnt) < 12:  # bruit
        return None
    return cnt
...
def _geom_feats(cnt):
    ...
    solidity = float(area) / hull_area  # ♥ a un creux -> plus faible que ♦
    ...
    if w < 1 or h < 1:
        ...
        ar = float(min(w, h) / max(w, h))  # ~1 pour ♦, plus petit pour ♥/♣/♠
        ang = abs(rect[2])
        if ang > 90:
            ang = 180 - ang
...
class SuitHu:
    """
    Classif de symbole par k-NN (k=1) sur Hu + tie-break géométrique (angle/aspect/solidity).
    - Templates lus dans assets/templates/suits/ :
        • h/*.png, d/*.png, s/*.png, c/*.png
        • et aussi h_*.png, d_*.png ... à la racine du dossier
    - Recommandé : ≥ 2–3 templates nets par suit (ta table, suit_rel serré).
    """
        ...
        for lab in ["h", "d", "s", "c"]:
            ...
            for p in files:
                try:
                    ...
                    continue
        # si self.db vide → on tombera sur les fallbacks géométriques

    # --- classification ---
    def classify(self, suit_patch_rgb, color_hint: Optional[str] = None) -> Tuple[Optional[str], float, Dict]:
        """
        Retourne (code, confidence, meta). color_hint ∈ {"red","black",None}
        """
        ...
        # Pas de templates utilisables -> fallback géométrique
        if best is None:
            if color_hint == "red":
                # ♦ : angle ~45° (±15), aspect ~1, très convexe
                lab = "d" if (abs(geo["angle"] - 45) < 15 and geo["aspect"] > 0.75 and geo["solidity"] > 0.92) else "h"
                return lab, 0.6, {"reason": "geom_fallback_red", **geo}
            if color_hint == "black":
                # ♣ vs ♠ : sans templates c'est dur -> heuristique rondeur/convexité pour ♣
        ...
        # Confiance à partir des distances
        d1, lab1, geo1 = best
        d2 = second[0] if second is not None else d1 + 1.0
        margin = max(0.0, d2 - d1)

        conf_dist = 1.0 / (1.0 + d1)  # ↓d1 → ↑conf
        conf_margin = min(1.0, margin / (d1 + 1e-6 + 0.5))
        conf = 0.5 * conf_dist + 0.5 * conf_margin

        # Tie-breaks si conf faible/marge faible
        if color_hint == "red":
            # ♦ : angle ~45° (±15), aspect ~1, très convexe

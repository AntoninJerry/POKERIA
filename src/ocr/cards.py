...
# ---------- Utils ----------
def _nonempty(img): return img is not None and hasattr(img,"size") and img.size>0 and img.shape[0]>0 and img.shape[1]>0
def _is_small_roi(w,h): return w<40 or h<40 or (w*h)<=2000
...
def read_card(engine: EasyOCREngine, crop_rgb, roi_name: Optional[str]=None):
    ...
    # 1) coin TL uniquement si ROI = carte entière ; sinon on garde la petite ROI telle quelle
    h,w = crop_rgb.shape[:2]
    corner = crop_rgb.copy() if _is_small_roi(w,h) else crop_rgb[0:int(0.55*h), 0:int(0.60*w)].copy()
    if not _nonempty(corner):
        return None, {"roi_name": roi_name, "error":"corner"}

    # 2) Rank (multi-pass)
    r_code, r_conf, r_meta = _read_rank(engine, corner)

    # 3) Suit par forme (Hu) guidé par la couleur (rouge/noir)
    color_hint = _suit_by_color(corner)  # "red" / "black"
    sp = _suit_patch(corner)
    if not _nonempty(sp):
        s_code = "h" if color_hint == "red" else "s"
        s_conf = 0.5
        s_meta = {"reason": "no_suit_patch"}
    else:
        s_code, s_conf, s_meta = _SUITS_HU.classify(sp, color_hint=color_hint)
    if s_code is None:
        s_code = "h" if color_hint == "red" else "s"
        s_conf = 0.5
    ...
    s_code, s_conf, s_meta = _SUITS_HU.classify(sp, color_hint=color_hint)
    if s_code is None:  # fallback sur la couleur seulement
        s_code = "h" if color_hint=="red" else "s"
        s_conf = 0.5
        s_meta = {"reason":"color_fallback"}

    # 4) Q vs 9 tie-break si conf faible
